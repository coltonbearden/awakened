...
## 7. Phase 5 — Evaluation Matrix Consolidation → Independent-Reviewer Approval Gate ✔ Complete
...
**Harness spike (2026-08-27, `eval/shortlist.md` §9 step 2 — SPEC v2.13).** Executed before any agent or hook was authored, from a throwaway marketplace outside the repository on Claude Code 2.1.247. Two findings, both recorded under D-24 (ADR-024 amended in place): **SPEC-GAP-002 closed — `tools` does not honour `Bash(<cmd>:*)`**, the subagent receives the whole tool (transcript-verified); and **the shell-free hook handlers D-24 mandates are unsupported on `SessionStart` and `SessionEnd`**, so neither budgeted hook ships at v1 and the two jobs ship as a skill and `/rinnegan:capture` under D-04. The one `defer` row resolved at T-287 — matrix **96 shortlist / 211 reject / 15 merge / 0 defer**. V6.5 is satisfied at zero hooks (the budget is a ceiling). The Windows 11 leg ran later the same day once `claude.exe` had re-authenticated and confirmed both findings (T-288).

**Delivered (evidence: PRs #27 scaffold, #28 spike, #29–#37 one per plugin, #38 this record).** The full `SPEC.md` §3 tree: `.claude-plugin/marketplace.json`, nine manifests and changelogs, `tests/` fixtures, `.github/workflows/upstream-watch.yml`; **114 component files** — 53 skills, 21 commands, 27 `bankai` agents, three `aura` statusline `.sh`/`.ps1` twin pairs, two output styles, five Markdown reference siblings (`poneglyph`, EXC-1) — synthesized from the 96 shortlisted rows plus the `eval/shortlist.md` §6 original-work plans (`aura` in full; `instinct`'s `auditing-components` and `linting-components`); **zero hooks** (SPEC v2.13, D-24 as amended); `SOURCES.md` §Shipped Components with one row per component, `NOTICE` extended for the EXC-1 MIT notice and the claude-mem conceptual lineage; `CHANGELOG.md` per plugin.

**Verification (passed).** V6.1 every §3 entry present, none marked deferred; `--release` / `-Release` exit 0 on both platforms. V6.2 both validators `VALIDATE: PASS` on WSL2 and Windows 11, CRLF-normalised output byte-identical on every PR. V6.3 marketplace, nine manifests, 53 skill frontmatters and 27 agent definitions pass checks C1–C4 and `claude plugin validate .`; the C-2 regex is the schema rule reimplemented, per `CLAUDE.md` §9. V6.4 catalog lists exactly nine, names equal the Tier-1 set, every `plugins/<dir>` on disk is listed. V6.5 zero hooks (the D-15 budget is a ceiling; the harness finding is recorded above). V6.6 checks P1–P4 zero hits over 114 files; zero bare `Bash`/`Write`/`Edit` in any agent `tools` string; zero network calls, telemetry or artwork. V6.7 `claude plugin marketplace add coltonbearden/awakened` → install `kaioken@awakened` → `/kaioken:aside` answered in a `claude -p` session → uninstall → marketplace removed, a clean round trip on **WSL2 and Windows 11** (2026-08-27). V6.8 109 primary components, 109 with a `SOURCES.md` row (reference siblings are recorded on their skill's row); Apache-2.0 lineage (anthropics, claude-mem) in `NOTICE`. V6.9 CI green on every Phase-6 PR. V6.10 every shipped component maps to a `shortlist` row (96) or a §6 original-work plan (`aura` 9 files, `instinct` 2); the mapping is the `SOURCES.md` row.

**Gate G6:** **APPROVED (review)** 2026-08-27 under standing delegation D11 (§10 rule 2) — the executing agent's review of V6.1–V6.10 against the tree at this PR. The step §8's gate line reserves was then taken by the owner: **v0.1.0 tagged on `1c0f777` and released 2026-08-30**, the tag and release created by the executing agent at the owner's explicit instruction and saying so in their own text (the D-25-ack pattern). Release: `https://github.com/coltonbearden/awakened/releases/tag/v0.1.0`. The repository's visibility was not changed at that point; **it was made public on 2026-09-02 — marketplace live** — after the standing delegation was widened to cover publishing, alongside the v0.1.1 maintenance release (§13).

**Gate G6:** owner sign-off → tag the first release → marketplace live. All three steps are done: review 2026-08-27, v0.1.0 2026-08-30, public repository 2026-09-02.
...
| G6 | 2026-08-27 | **APPROVED (review)**; **v0.1.0 released 2026-08-30** (owner's instruction, executed and stated); **marketplace live 2026-09-02** | V6.1–V6.10 all discharged with measured values in §8; reviewed under standing delegation D11 (§10 rule 2). Review record: PR #38. The first release tag followed on 2026-08-30 and the repository went public on 2026-09-02 under the widened delegation; both recorded in §8 |
| G5 | 2026-08-27 | **OWNER ACK** | Acknowledgement comment on the sign-off PR #25: `https://github.com/coltonbearden/awakened/pull/25#issuecomment-5440726308`. §3.5 second pass waived by the owner in the same comment (SPEC v2.11; ADR-027 amended in place). Phase 6 opens on this row (D-25) |
...
- New plugin proposals — only via `SPEC.md` §5 Future Naming Logic (scope and boundaries first) plus a new ADR. **Decided 2026-09-02:** none open; a proposal is raised only from a measured gap — an upstream-watch diff or a user-filed issue — never speculatively.
- `aura` preset expansion beyond the §5 tables — original work, same review path. **Decided 2026-09-02:** not scheduled; it opens when a user-filed issue asks for a preset, and then under N-5 with no exceptions (D-28). **Amended 2026-09-03:** the six §5 palettes themselves now ship as files (`plugins/aura/palettes/`, `aura` 0.2.0, D-29) — that is the table realized, not an expansion beyond it; a seventh preset of either kind still waits for a user-filed issue.
- Visual identity — **Decided 2026-09-03:** owner-directed, executed and decided under the standing delegation: the Redline system in `brand/` (SPEC v2.20, D-29 / ADR-029), README banners, the social preview, and the palettes above; released as **v0.2.0** (`aura` 0.2.0, marketplace catalog 0.2.0, the eight other plugins unchanged). No website (D-02), no generative-image prompts, no new statusline presets.
...
- Listing on community catalogs — discovery, not a dependency. **Decided 2026-09-02:** not pursued; the public GitHub repository and its release page are the discovery surface.
- Upstream-watch tuning based on the first months of live diff issues. **Decided 2026-09-02:** the first cycle (issue #40 → T-289 re-pin → T-290 review) needed none — the diff named exactly the 13 audited files that changed; re-examined after the 2026-10-01 run.

The §11 loop has run once end to end: upstream-watch (2026-09-01, issue #40) → re-pin (T-289, PR #42) → dogfooded review of the changed audited files (T-290) → per-plugin changelog entries → **v0.1.1** (2026-09-02: `kaioken` and `super-saiyan` at 0.1.1, marketplace catalog 0.1.1, the seven untouched plugins stay 0.1.0). Each cycle is recorded the same way; a re-pin and a review are separate triage-log entries.
