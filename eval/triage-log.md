...
### T-282 — git-commit-guardian

- **Source:** davila7/claude-code-templates
- **Path:** cli-tool/components/agents/git/commit-guardian.md
- **HR/axis trigger IDs:** `n/a (re-audit)`
- **Rationale:** **Re-audit after the G5 round-2 spot-check** (`ROADMAP.md` §7), re-read in full at the pinned commit `8546d44fdec5c9775bf92ef881090820e568198e`. Previous: `4,4,5,5,5`, `shortlist`, `risk` 5 on the "read-only; no shell side effects" anchor. **The reviewer's finding was correct.** Line 14: "If everything passes, make the commit"; Check 3 (lines 30-37) detects and runs the project's build (`dotnet build`, `npm run build`, `python -m py_compile`, `go build`, `cargo check`); Check 4 runs the test suite; Check 5 (line 45) "Auto-fix if possible, re-stage, continue"; the all-pass example (line 59) is `git commit -m ...`. That is not read-only. The SPEC §9 anchor for 5 does not hold; 4 does - writes and commands inside the project, under the component's own Absolute Restrictions (never `--no-verify`, never handle secrets, never `git push`), with C-3 in bounds. The reviewer's secondary point is also right and is now stated in the row: the proposed allowlist `[Read, Grep, Glob, 'Bash(git status:*)', 'Bash(git diff:*)', 'Bash(git log:*)']` cannot run a build, a test suite, an auto-fix or a commit, so the `bankai` agent synthesis produces is the verification *report* (checks 1, 2, 6 and Conventional-Commits validation) and the commit stays with the user or `ecc/cmd-prp-commit`. Scoring the file as shipped and constraining the synthesis are two different statements; the rationale now makes both. **New: `4,4,4,5,5`, `shortlist` retained**, rationale replaced in place (§2 rule 4).
- **Date:** 2026-08-25

### T-283 — cmd-prp-implement

- **Source:** affaan-m/ECC
- **Path:** commands/prp-implement.md
- **HR/axis trigger IDs:** `n/a (re-audit)`
- **Rationale:** **Re-audit after the G5 round-2 spot-check**, re-read at the pinned commit `06c5e118c4d3e6c3b7f9445f973a2194c82de193`. Previous: `4,3,4,4,4`, `shortlist`. **The reviewer's disclosure finding was correct.** Under "Sync Remote" (line 92) the command runs `git pull --rebase origin $(git branch --show-current) 2>/dev/null || true` unprompted, with stderr discarded and failure swallowed, immediately before a CHECKPOINT that reads "Remote synced" - a rebase that can leave a mid-rebase tree while reporting success. The earlier rationale stated the HR-4 and HR-6 clearances for the Level-4 dev-server fixture and said nothing about this step. **Adjudication.** The write is inside the project and the command is ambient git, the same reading that scored `ecc/cmd-prp-commit`'s git-only shell at `dependencies` 4; no HR fires and no score moves. What was missing is disclosure and a constraint: synthesis MUST replace the silent rebase with a confirmed, non-silent sync (or drop the step), because a swallowed `|| true` on a history-rewriting command is exactly the shape `superpowers/verification-before-completion` exists to forbid. **New: `4,3,4,4,4`, `shortlist` retained**, rationale replaced in place (§2 rule 4) to disclose line 92 and carry the constraint.
- **Date:** 2026-08-25
...
- **Path:** skills/search-first/SKILL.md
- **HR/axis trigger IDs:** `n/a (re-audit)`
- **Rationale:** **Re-audit after the G5 round-2 spot-check**, re-read at the pinned commit `06c5e118c4d3e6c3b7f9445f973a2194c82de193`. Previous: `4,4,4,5,4`, `shortlist`. **The reviewer's disclosure finding was correct.** Step 5 IMPLEMENT reads "Install package / Configure MCP / Write minimal custom code" (line 49) and the decision matrix's Adopt and Extend rows read "install and use directly" and "install + write thin wrapper" (lines 58-59); no confirmation gate precedes the install. The earlier rationale constrained only the Configure-MCP branch. **Adjudication.** HR-7 is "auto-installing packages or runtime dependency fetching" by the *component* for itself; here the install is the user's own project adopting a library the user asked the skill to find - user-directed dependency adoption, the D-24-era distinction the matrix has applied since Phase 2 (`gateguard`'s documented optional install cleared; `superpowers/using-git-worktrees`' unprompted `npm install` did not). No HR fires and no score moves. The constraint that was missing is now in the row: synthesis MUST put an explicit confirmation before any install, alongside the existing MCP-branch strip. **New: `4,4,4,5,4`, `shortlist` retained**, rationale replaced in place (§2 rule 4).
- **Date:** 2026-08-25
...
- **HR/axis trigger IDs:** `n/a (re-audit)`
- **Rationale:** **Re-audit after the G5 escalation** (`ROADMAP.md` §7). The second G5 reviewer found that on T-280's wording - "a process the component starts that returns immediately and runs on after the turn is a background worker" - this row's "Spin up a **background agent** to do the research, so you keep working while it reads" (line 6, pinned commit `9c9f36ccd3995266cd675468af71639c8dde1ec5`) would fire HR-4, and that the artifacts drew no line between that and a detached `claude --bg` process. The project owner drew the line on 2026-08-25 and it is now `SPEC.md` §6's HR-4 note (v2.9): a subagent the harness runs *inside* the session, returning its output to the conversation and ending with the turn or the session, is the harness's own mechanism and not HR-4; a process detached from the session and managed outside it is. **Adjudication.** This skill's agent is the first kind - it runs within the session and its product is a Markdown file written into the repository (C-3), with nothing to manage afterwards; `claude-handoff`'s was the second kind, a `claude --bg` job managed through `claude agents` (T-280 stands). Previous and new: `4,5,4,5,5`, `shortlist` - unchanged; rationale replaced in place (§2 rule 4) to state the line and the constraint that synthesis keeps the dispatch inside the session. The same note settles the question for every `bankai` dispatch row and `superpowers/dispatching-parallel-agents` without further entries.
- **Date:** 2026-08-25

### T-286 — agent-security-reviewer

- **Source:** affaan-m/ECC
- **Path:** agents/security-reviewer.md
- **HR/axis trigger IDs:** `HR-7`
- **Rationale:** **Re-audit after the G5 escalation.** Previous: `4,4,4,4,3`, `shortlist`, cleared on "not the every-step-is-npx case". The second G5 reviewer found the Initial Scan unconditionally runs `npx eslint . --plugin security` (lines 33-34, 40 at the pinned commit `06c5e118c4d3e6c3b7f9445f973a2194c82de193`) with no detection gate of the kind that cleared `ecc/cmd-test-coverage`, and that T-106/T-136's every-step reading and T-279's any-trigger reading could not both stand. The project owner settled it on 2026-08-25 and it is now `SPEC.md` §6's HR-7 note (v2.9): the test is whether the invocation would fetch; a single unconditional fetching step is enough. **Adjudication.** `npx eslint . --plugin security` is gated on nothing - the agent does not check that the project declares `eslint`, and `eslint-plugin-security` is not a project runner by any reading - so it would fetch in any project that does not already carry both, and HR-7 fires. `npm audit --audit-level=high` alone would have cleared: it runs against the project's own lockfile and fetches no tool. The rest of the file is prompt-level pattern review, which is why the earlier clearance was tempting; a hard reject overrides scores. **New: `4,4,4,4,3`, `hard_reject = HR-7`, `reject`.** Re-donated: the OWASP Top 10 sweep and the Common False Positives section to `sharingan`'s review rows; the secret-detection patterns join T-146's library; the emergency-response path to `ecc/agent-code-reviewer`. Consistency check under the same note: `davila7/development-tools-unused-code-cleaner` (T-279, `npx depcheck`) stays rejected; `ecc/cmd-build-fix`, `ecc/cmd-test-coverage` and `ecc/cmd-code-review` stay cleared - each `npx` there is gated on the project's own config or runner. `bankai` drops to 34 shortlisted rows.
- **Date:** 2026-08-25

### T-287 — session-memory (concept)

- **Source:** thedotmack/claude-mem
- **Path:** docs/architecture-overview.md
- **HR/axis trigger IDs:** `n/a (re-audit)`
- **Rationale:** **Re-audit closing the one `defer` row at the Phase-6 harness spike.** Previous: `5,4,2,5,5`, `defer` (T-281) naming C-1 — the idempotence and timeout of the `rinnegan` capture hook designed in `eval/claude-mem-rebuild.md` §3.1 had to be executed on both platforms before the hook shipped. The spike executed the design's handler forms against Claude Code 2.1.247 from a throwaway marketplace outside the repository (WSL2, `claude -p` and an interactive session): a `prompt`-type `SessionStart` handler failed with `prompt-type hooks are not supported for SessionStart events (no conversation context is available). Use a command-type hook instead.`, an `agent`-type `SessionStart` handler failed with the same message for its type, and an `agent`-type `SessionEnd` handler failed with `Agent stop hooks are not yet supported outside REPL` in both session kinds. `SPEC.md` §6 Hook Dispatch (D-24) admits no `command` handler, so the hook the check attached to cannot exist in any form the spec permits. **Adjudication.** The named check is discharged, not passed: C-1 binds hooks, and no hook ships. The design survives whole as the `/rinnegan:capture` command — `eval/claude-mem-rebuild.md` §3.2 is its procedure and §3.3's hash idempotence holds for a command as for a hook — which is the higher D-04 form; a `defer` row whose blocking check has become inapplicable resolves on the design it scores (`eval/rubric.md` §7 Example C names the phase, and this is that phase). **New: `5,4,3,5,5`, `shortlist`** — `risk` returns to the pre-T-281 value, the hook-idempotence concern that lowered it being moot. The same spike closed SPEC-GAP-002 (the subagent granted `Bash(git ls-files:*)` ran `git status --short` and `echo` undenied and reported `TOOLS-VISIBLE: Read, Grep, Glob, Bash`; transcript-verified, not self-reported); that outcome is recorded in `SPEC.md` v2.13 §14 and ADR-024, and binds every `bankai` agent's allowlist at synthesis. **Limit stated:** the Windows 11 leg did not run — `claude.exe` returned `401 OAuth access token has expired`, which only the owner can clear — and is re-run before G6; both findings are harness behaviour on one CLI version, not platform behaviour, and the WSL2 record is what this entry rests on. Matrix: 322 rows, **96 shortlist / 211 reject / 15 merge / 0 defer**; `rinnegan` 3.
- **Date:** 2026-08-27

### T-288 — session-memory (concept) — Windows 11 leg of the harness spike

- **Source:** thedotmack/claude-mem
- **Path:** docs/architecture-overview.md
- **HR/axis trigger IDs:** `n/a (re-audit)`
- **Rationale:** **Record closing the stated limit of T-287.** T-287 rested on the WSL2 execution alone because `claude.exe` had returned `401 OAuth access token has expired`. On 2026-08-27, after the V6.7 install smoke test showed `claude.exe` authenticated again, the same spike was executed on Windows 11 (PowerShell 7, Claude Code 2.1.247, a throwaway git repository under the user's temp directory): the subagent declaring `tools: Read, Grep, Glob, Bash(git ls-files:*)` ran `git ls-files`, `git status --short` and `echo PROBE-ECHO` with no denial and reported `TOOLS-VISIBLE: Read, Grep, Glob, Bash`; the `agent`-type `SessionEnd` handler failed with `Agent stop hooks are not yet supported outside REPL`. Both findings therefore hold on both platforms. **No score or verdict changes** — T-287's `5,4,3,5,5` / `shortlist` stands; this entry exists so the record does not rest on one platform. The throwaway marketplace and plugin were uninstalled from both machines afterwards.
- **Date:** 2026-08-27

### T-289 — 2026-09 re-pin (issue #40, first monthly upstream-watch run)

- **Source:** all ten `SPEC.md` §8 repositories
- **Path:** upstream.json
- **HR/axis trigger IDs:** `n/a (re-pin)`
- **Rationale:** **Re-pin record, owner-instructed.** The first monthly `upstream-watch` run (2026-09-01, issue #40) found eight of ten pins behind their default-branch HEADs. On the owner's instruction the pins were advanced on 2026-09-02 by `scripts/pin-upstream.sh` (`PIN: PASS (10/10 resolved)`, `pinned_at` 2026-09-02T15:34:36Z); `obra/superpowers` and `kepano/obsidian-skills` resolved to their existing pins. **Affected evaluations, measured** by `git diff --name-only <old-pin>..<new-pin>` in fresh fetches of each clone, intersected exactly against the upstream paths of the 96 shortlisted rows: **13 audited source files changed** — 10 `mattpocock/skills` skills (`code-review`, `codebase-design`, `diagnosing-bugs`, `domain-modeling`, `prototype`, `research`, `resolving-merge-conflicts`, `tdd`, `grilling`, `writing-for-agents`) and 3 `affaan-m/ECC` commands (`learn-eval`, `resume-session`, `skill-create`); zero audited files changed in `thedotmack/claude-mem`, `wshobson/agents`, `anthropics/skills`, `davila7/claude-code-templates`; `vercel-labs/skills` and `hesreallyhim/awesome-claude-code` hold no scored rows (discovery/concept roles). **No verdict, score, or shipped component changes**: every evaluation and every synthesis is grounded at the pin its matrix, shortlist and `SOURCES.md` rows cite, which are historical records; adopting an upstream change is a separate deliberate act under §11's dogfooded review, starting from the 13 files named here. **Licenses re-verified at the new pins** (ADR-024 enforcement): all ten unchanged against §8 — MIT where §8 says MIT, `thedotmack/claude-mem` still carries its root Apache-2.0 `LICENSE` (untouched by the diff; a first-line-blank artifact in the check script briefly misread it as absent and is corrected here rather than propagated), `anthropics/skills` still has no root license and `skills/discernment-nudge/LICENSE.txt` still exists, `hesreallyhim/awesome-claude-code` still CC-BY-NC-ND-4.0 (discovery-only). Issue #40 closes on this entry.
- **Date:** 2026-09-02

### T-290 — 2026-09 upstream deltas (first §11 dogfooded review of the T-289 files)

- **Source:** mattpocock/skills, affaan-m/ECC
- **Path:** skills/engineering/{code-review,codebase-design,diagnosing-bugs,domain-modeling,prototype,research,resolving-merge-conflicts,tdd}/SKILL.md, skills/productivity/{grilling,writing-for-agents}/SKILL.md; commands/{learn-eval,resume-session,skill-create}.md
- **HR/axis trigger IDs:** `n/a (re-audit)`
- **Rationale:** **Re-audit of the 13 audited source files T-289 named as changed between the 2026-08-18 and 2026-09-02 pins**, read as `git diff <old-pin>..<new-pin>` per file in fresh blobless clones. **mattpocock/skills** (`9c9f36cc`→`6654f6b6`): the range holds three commits touching these files — an em-dash removal sweep, a YAML fix quoting `description` values that contain colons, and one change to `grilling`. In nine of the ten files every removed line carries an em-dash and every added line is its re-punctuated form or the re-quoted description: `code-review`, `codebase-design`, `diagnosing-bugs`, `domain-modeling`, `prototype`, `research`, `resolving-merge-conflicts`, `tdd`, `writing-for-agents` — **no technique, step, tool, or safety property changed; no action** (the sibling reference files in those directories changed the same way). `grilling` additionally changed its round template to place a horizontal rule between consecutive questions — a readability convention with no new behaviour; **adopted in our own words** as one workflow clause and one Question Shape row in `plugins/super-saiyan/skills/grilling/SKILL.md`. **affaan-m/ECC** (`06c5e118`→`ca185ef5`): `learn-eval` changed one sentence of its design-rationale prose (a model-generation reference) — no action; `skill-create` corrected its frontmatter key `allowed_tools`→`allowed-tools` — our command already used the documented key, no action; `resume-session` gained a candidate-ranking rule for implicit and date-based lookup (reject empty, whitespace-only, placeholder-only and one-message-echo session files; deterministic tie-breaks; an explicit path is never substituted). The concept closes a real gap in our counterpart, which picked "the newest file" and could therefore brief from a half-written handoff; **adopted in our own words** as an Eligibility section in `plugins/kaioken/commands/resume-session.md`, mapped onto our own file format (index rows, file-state table, failed approaches, next step) rather than ECC's. Both adoptions were checked against `instinct`'s `auditing-components` procedure: neither adds a tool, command, write target, environment variable, or endpoint to its bill of materials, and neither directs the reading agent to skip a confirmation — PASS on both; the `resume-session` change narrows behaviour (fewer files are loadable implicitly) and the `grilling` change is presentational. **No verdict, score, or matrix change**; `SOURCES.md` rows for both components cite the re-audit pin beside the synthesis pin (D-12), and each plugin's `CHANGELOG.md` records the change under `[Unreleased]`. Matrix: 322 rows, 96 shortlist / 211 reject / 15 merge / 0 defer.
- **Date:** 2026-09-02

## 4. Statistics

Recomputed at each gate from the entries in §3, so the table can be checked against the file rather than trusted. The first four rows partition the entries by their trigger field under one stated precedence rule: an entry carrying **any** `HR-N` counts as hard-reject; otherwise any `axis:` trigger counts as axis-floor; otherwise a rule ID such as `B-1..B-8`, `D-15` or `D-24` counts as other-rule; a field beginning `n/a` is its own row **including the `n/a (re-audit)` form**, which the v2.6 table wrongly filed under other-rule (T-069). 47 + 148 + 37 + 58 = 290.

| Metric | Count |
|---|---|
| Total entries | 290 |
| Hard-reject entries | 47 |
| Axis-floor entries | 148 |
| Other-rule entries | 37 |
| `n/a` entries | 58 |
| Bulk-reject classes | 19 |
| Gap-scan entries | 18 |
| Re-audit / re-pin entries | 27 |

The last three rows **overlap** the partition above and are not added to it: the nineteen bulk-reject classes are T-029…T-037, T-070…T-072 and T-151, T-152, T-154 plus the T-150 and T-240 scoping records and T-241, T-242 from pass 3, thirteen of which also carry an `HR-N` or `axis:` trigger; the eighteen re-audit entries are T-026…T-028 (the 2026-08-18 G5 rehearsal), T-064…T-069 (the 2026-08-22 review corrections), T-149 (the B-6 agent-ownership recalibration), T-273 (the resolvable-citation and trigger-coherence sweep), T-274, T-275 (the Phase-5 re-examination of the two knowingly-contested shortlist rows) T-277…T-281 (the G5 round-1 remediation, three of which are also rejects counted in the hard-reject and axis-floor rows) T-282…T-284 (the G5 round-2 corrections) and T-285, T-286 (the owner's two escalation decisions applied; T-286 is also a reject counted in the hard-reject row), and T-287, T-288 (the Phase-6 harness spike closing the one `defer` row, and its Windows 11 leg), and T-289, T-290 (the 2026-09 re-pin and the first dogfooded review of the files it named). The words "sixteen", "sixteen" and "ten" that stood in this sentence until 2026-08-25 were stale against the table and are corrected at T-276. **Gap-scan entries move off 0 for the first time**: T-221…T-238 disposition all 157 rows of `hesreallyhim/awesome-claude-code`'s catalog across its 18 categories — 9 mapped to an owning plugin, 9 explicitly out of scope, zero merge candidates sourced from it (V4.4).
...
**Gate G5 round 2 (2026-08-25) returned `REJECTED` again and added T-282…T-284**: the second reviewer's spot-check read twenty-two shortlisted rows and raised five findings. Three were corrected in place with no verdict moved - `davila7/git-commit-guardian` `risk` 5→4 (it builds, tests, auto-fixes and commits), and undisclosed steps in `ecc/cmd-prp-implement` (a silent `git pull --rebase ... || true`) and `ecc/search-first` (an unconfirmed install) now stated with constraints. The other two are **not** adjudicated here because the reviewer's own text says the artifacts draw no line for them and a second `REJECTED` escalates the gate to the project owner (`eval/gate-review-protocol.md` §5): whether a *session-scoped* background subagent (`mattpocock/research`, and by extension every `bankai` dispatch row) falls under HR-4 as T-280 applied it to a detached `claude --bg` process, and whether a single `npx <analysis-tool>` step (`ecc/agent-security-reviewer`'s `npx eslint . --plugin security`) fires HR-7 under T-279's any-trigger reading or only under T-106's every-step reading. Both are recorded as open at `ROADMAP.md` §7 and decided by the owner. `eval/matrix.csv` stays at 322 rows, 96 / 210 / 15 / 1.

**The owner decided both on 2026-08-25 (SPEC v2.9 §6 notes; T-285, T-286).** HR-4 stops at the session boundary, so `mattpocock/research` is retained with the line stated; HR-7 is a fetch test, so `ecc/agent-security-reviewer` moves to `reject`. `eval/matrix.csv`: 322 rows, **95 shortlist / 211 reject / 15 merge / 1 defer**. The owner also chose route (a) of the escalation - to adjudicate G5 personally on the corrected artifacts - and that verdict is recorded at `ROADMAP.md` §7 and §11 when given.
...
**Phase 6 (2026-08-27) added T-287**, the harness spike's re-audit: the one `defer` row moves to `shortlist` because the hook its blocking check attached to cannot exist in any form `SPEC.md` §6 permits, and the design ships as a command. `eval/matrix.csv` is unchanged in size and now reads **322 rows, 96 shortlist / 211 reject / 15 merge / 0 defer**.

**Phase 6 (2026-08-27) added T-288**, the Windows 11 record of the harness spike: no verdict moves; the T-287 findings now rest on both platforms.

**Maintenance (2026-09-02) added T-289**, the first §11 re-pin: eight pins advanced by `scripts/pin-upstream.sh` at the owner's instruction, 13 audited source files identified as changed upstream, no verdict moves, all ten licenses re-verified unchanged.

**Maintenance (2026-09-02) added T-290**, the first §11 dogfooded review of those 13 files: eleven dispositioned as no-action (typographic sweeps, a quoting fix, a prose sentence, a frontmatter key our copy already used), two concepts adopted in our own words (`grilling` round separator; `resume-session` candidate eligibility), no verdict moves.
